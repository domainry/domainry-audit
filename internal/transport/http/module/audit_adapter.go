...
func NewAuditHTTPAdapter(application *auditapp.AuditQueryApplicationService, businessExport bool) (modulehttp.Adapter, error) {
	handler, err := NewAuditHandler(application)
	if err != nil {
		return nil, err
	}
	adapter := &AuditHTTPAdapter{handler: handler, mux: http.NewServeMux(), businessExport: businessExport}
	...
	if businessExport {
		handlers[auditapp.AuditPermissionBusinessExportPrepare] = handler.prepareBusinessExport
		handlers[auditapp.AuditPermissionBusinessExportDownload] = handler.downloadBusinessExport
	}
	...
	if len(handlers) != 0 {
		...
		return nil, fmt.Errorf("Audit HTTP handlers have no Action route entries: %v", keys)
	}
	return adapter, nil
}
...
func (s *AuditHTTPAdapter) Routes() []modulehttp.Route {
...
func AuthorizationActions(businessExport ...bool) []actioncontract.ActionDefinition {
	enabled := true
	if len(businessExport) != 0 {
		enabled = businessExport[0]
	}
	routes := (&AuditHTTPAdapter{businessExport: enabled}).Routes()
